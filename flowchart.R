...
# define the global styles of the nodes. We can override these in box if we wish
node [shape = rectangle, style = filled, fillcolor = gray, color = orange, fontname = Courier, height=10, fontsize = 50]

data1 [label = 'An analysis of genetic differentiation based on Fst estimates revealed two major genetic clusters; one with all the American samples grouped together, \n and the other one grouping the European samples. Similarly, there is some degree of genetic differentiation among samples within each continent, \n suggesting a total of 6 ancestral populations k with unique patterns of allelic variation in the for gene. The results indicate that \n different patterns of gene flow as expected under models of isolation by environment (IBE) and isolation by distance (IBD) \n are likely driving the genetic differentiation among populations of D. melanogaster.', shape = invhouse, fillcolor = Beige, fontcolor = purple]
